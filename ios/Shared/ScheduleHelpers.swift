...
struct ScheduleHelpers {
    ...
    // MARK: - 周过滤
    /// 过滤出在指定周数有效的课程
    static func courses(forWeek weekNumber: Int, courses: [Course]) -> [Course] {
    ...
    /// 过滤出某天的课程，按开始时间排序
    static func courses(forDay dayOfWeek: Int, courses: [Course]) -> [Course] {
    ...
    static func currentAndNext(courses: [Course], at now: Date = Date()) -> (current: Course?, next: (course: Course, startDate: Date)?) {
