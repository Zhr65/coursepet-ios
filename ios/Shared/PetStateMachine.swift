...
struct PetActionResult {
    let action: String      // 动作 ID，对应 pet_assets 中的文件名前缀
    let bubble: String      // 气泡文字
}

struct PetStateManager {
    ...
    /// 根据心情返回气泡文字
    static func bubble(for mood: Int, random: @escaping () -> Double = { Double.random(in: 0..<1) }) -> String {
    ...
    static func decideAction(
        ...
        var ended: [(Course, Int)] = []
        for course in courses where course.dayOfWeek == dow {
            if let e = ScheduleHelpers.timeToMinutes(course.endTime), e <= curMin {
                ended.append((course, e))
            }
        }
        ended.sort { $0.1 > $1.1 }  // 按结束时间降序
